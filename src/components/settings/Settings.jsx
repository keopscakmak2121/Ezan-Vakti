                ...
                {expandedPrayer === key && (
                  <div style={styles.prayerContent}>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <select style={{...styles.select, width: '100%', marginBottom: '8px'}} value={soundId} onChange={(e) => handlePrayerSettingChange(key, 'soundId', e.target.value)}>
                        {(SOUND_OPTIONS[soundType] || []).map(s => (
                          <option key={s.id} value={s.id}>{s.name} {!isSoundDownloaded(s.id) && s.id !== 'default' ? '☁️' : ''}</option>
                        ))}
                      </select>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button onClick={() => handlePreviewSound(soundId, soundType)} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', backgroundColor: playingSound === soundId ? '#dc2626' : '#059669', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '13px', fontWeight: '500' }}>
                          {playingSound === soundId ? <><Square size={14} fill="white" /> Durdur</> : <><Play size={14} fill="white" /> Dinle</>}
                        </button>
                        {!isDownloaded && soundId !== 'default' && (
                          <button onClick={() => handleDownload(soundId, soundType)} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', backgroundColor: '#3b82f6', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '13px', fontWeight: '500' }}>
                            {downloadingId === soundId ? '⏳ İndiriliyor...' : <><Download size={14} /> İndir</>}
                          </button>
                        )}
                      </div>
                    </div>
